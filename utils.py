...
def TPFP_calculator(X, Y, threshold):
    """
    X: Positive Class dataset
    Y: Negative Class dataset
    thresholds: A list of thresholds for calculating TP and FP rates
    """
    rates = [0, 0, 0, 0]
    #       [TP, FP, FN, TN]
...
def best_threshold(true_pos, false_pos, acc, thresholds):
    ...
    for i in range(len(thresholds)):
        if (true_pos[i] - false_pos[i]) / 2 > best_rate:
            ...
            best_fp = false_pos[i]
            best_acc = acc[i]
    return {'best TP rate': best_tp, 'best FP rate': best_fp, 'best accuracy': best_acc,
...
def caoICME10(I, d, tau):
    ...
    #    tau: A threshold to determine a region is textured or not
